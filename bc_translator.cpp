...
Status* BytecodeTranslator::translate(const string& program, Code** codePtr) {
    ...
    return translate(parser.top(), codePtr);
}

Status* BytecodeTranslator::translate(AstFunction* top, Code** codePtr) {
#   if DEB
...
bool BytecodeTranslator::checkRecCall(AstNode* node) {
    if (node->isCallNode()) {
